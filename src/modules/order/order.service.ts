  ...
  for (const it of cart.items) {
    ...
    if (it.variantSku) {
      const variant = product.variants?.find(
        (v: any) => String(v.sku) === String(it.variantSku)
      );

      if (!variant) {
        throw new AppError(400, `Variant not found`);
      }
    ...
    const shipping = product.shipping?.freeShipping
      ? 0
      : product.shipping?.shippingFee ?? 0;
